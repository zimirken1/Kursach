  ...
  const { data } = useQuery({
    queryKey: [
      activeTab === Tabs.MyExercises
        ? exerciseQueryKeys.MY_EXERCISES
        : exerciseQueryKeys.EXERCISES,
    ],
    queryFn: () =>
      ExerciseApi.getExercises({
        filter:
          activeTab === Tabs.MyExercises
            ? ExerciseFilter.MY_EXERCISES
      ...
      <FlatList
        ...
        ListFooterComponent={
          activeTab === Tabs.MyExercises ? (
            <Button
              onPress={() =>
                router.push('/trainings/create_workout/add_exercises/form')
              }
          ...
          <ExercisePreviewCard
            ...
            onPress={() => {
              setSelectedExercisesIds((prev) => {
                if (prev.includes(data.item.id)) {
                  return prev.filter((id) => id !== data.item.id)
                }
                return [...prev, data.item.id]
              })
              router.back()
            }}
            selected={selectedExercisesIds.includes(data.item.id)}
          />
        )}
      />
    </View>
